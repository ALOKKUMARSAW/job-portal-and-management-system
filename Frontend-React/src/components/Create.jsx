...
const Create = () => {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post("http://localhost:8080/jobPost", submitData)
      .then((resp) => {
  ...
  const { postId, companyName, companyLogoText, postProfile, reqExperience, postingDate } = form;
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 4 }}>
          <AddCircleIcon sx={{ fontSize: 40, color: '#667eea' }} />
          <Typography
            variant="h4"
            sx={{
              fontWeight: 800,
